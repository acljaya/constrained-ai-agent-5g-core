"""
AI Agent Network Partition Chaos Orchestrator
"""
import sys
import time
import json
...
class NetworkPartitionOrchestrator:
    def __init__(self):
        self.agent = AIAgentFair(use_fast_path=False)
        self.results = []
        
    def run_experiment(self, trials=10):
        """Run network partition chaos experiments"""
        
        print(f"\n{'='*60}")
        print(f"AI AGENT CHAOS EXPERIMENT: NETWORK PARTITION")
    ...
    def _single_trial(self, trial_num):
        ...
        print(f"[{fault_time.isoformat()}] Applying network partition...")
        ...
        fault_data = {
            ...
            "details": f"Pod isolated from cluster. Readiness probe failures: {probe_failures}"
        }
        ...
        # Wait for chaos to complete (25s duration)
        print("\nWaiting 25s for chaos to complete...")
        time.sleep(20)  # Already waited 5s
        ...
        try:
            subprocess.run(
                ["kubectl", "delete", "-f", "../chaos-scenarios/network-partition.yaml"],
                check=True,
                capture_output=True
            )
...
if __name__ == "__main__":
    orchestrator = NetworkPartitionOrchestrator()
    
    print("Running AI Agent Network Partition Chaos Experiments")
    
    results = orchestrator.run_experiment(trials=10)
    
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"../results/ai_chaos_partition_{timestamp}.json"
